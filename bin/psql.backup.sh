...
# -
  ...
  if [[ ${compressed} -eq 1 ]]; then
    write_yellow "Executing>> PGPASSWORD=${rs_password} pg_dump -Fc ${rs_verbose} -h ${rs_server} -p ${rs_port} -U ${rs_username} -d ${rs_database} ${rs_table} >> ${rs_directory}/${rs_filename} 2>/dev/null"
    ...
    write_yellow "Executing>> PGPASSWORD=${rs_password} pg_dump -Fp ${rs_verbose} -h ${rs_server} -p ${rs_port} -U ${rs_username} -d ${rs_database} ${rs_table} >> ${rs_directory}/${rs_filename} 2>/dev/null"
  ...
  if [[ $? == 0 ]]; then
    write_yellow "Database backup saved to ${rs_directory}/${rs_filename}"
  else
